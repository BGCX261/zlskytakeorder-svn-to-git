 ...
 * @param $toTime
 */
function getToDate($time = NULL, $toTime = NULL,$format = 'Y-m-d H:i:s') {
	if (empty($time))
		return '';
	if (is_null ( $toTime ))
		return (is_numeric($time))?date ( $format, $time ):date ( $format, strtotime($time) );
	$time=strtotime($toTime,$time);
