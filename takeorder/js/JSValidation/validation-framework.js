...
//需要验证区域
var ValidationRoot = "http://192.168.30.30/new_ata/js/JSValidation/";
...
XmlDocument.create = function () {
	if (document.implementation && document.implementation.createDocument) {
		...
		/**
		*
		*原因: 由于原验证框架的源文件validation-framework.js存在缺陷,
		*导致浏览器无法正确创建DOM Document对象
		*/
		var obj = null;
		...
		if (obj == null)
			throw new Error("你的浏览器不支持DOM Document!");
	} else
		throw new Error("你的浏览器不支持DOM Document!");
}
...
ValidationFramework.validateDepend = function(field, depend) {
	if (depend.getName() == "required") {
	...
	} else if (depend.getName() == "legalchar") {
		return ValidateMethodFactory.validateEnglishChinese(field, depend.getParams());
	} else if (depend.getName() == "englishchar") {
		return ValidateMethodFactory.validateEnglishChar(field, depend.getParams());
		...
		return ValidateMethodFactory.validatePersonalID(field, depend.getParams());
    } else if(depend.getName() == "phoneNumber"){
		return ValidateMethodFactory.validatePhoneNumber(field, depend.getParams());
    }else {
...
ValidateMethodFactory.validateEnglishChinese = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if(obj==null)
	    return false;
	if(obj.value=="")
	    return false;
	var exp = new RegExp("^[A-Za-z0-9_]*$|^[\u4E00-\u9FA5\uF900-\uFA2DA-Za-z0-9_]*$");
	//var exp = new RegExp("^[A-Za-z0-9_]*$");
	return exp.test(obj.value);
}
ValidateMethodFactory.validateEnglishChar = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if(obj==null)
	    return false;
	if(obj.value=="")
...
ValidateMethodFactory.validatePersonalID = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if(obj==null)
	    return false;
	if(obj.value=="")
	    return false;
	var exp = new RegExp("^\\d{15}$|^\\d{18}$|^\\d{17}[xX]$");
	return exp.test(obj.value);
}
ValidateMethodFactory.validatePhoneNumber = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if(obj==null)return false;
	if(obj.value=="") return false;
alert(obj.value);
	var exp = new RegExp("^[01]?\s*[\(\.-]?(\d{3})[\)\.-]?\s*(\d{3})[\.-](\d{4})$");
alert(exp);
        return exp.test(obj.value);
}
...
ValidationErrorString["zh-cn"]["pid"]="{0}号码应为15或18位!";
ValidationErrorString["zh-cn"]["email"]="Email格式有误，请重新输入";
ValidationErrorString["zh-cn"]["equalsfield"]="{0}和{1}不一致，请重新输入";
ValidationErrorString["zh-cn"]["phoneNumber"]="{0}不是一个有效电话号码!";
ValidationErrorString["zh-cn"]["legalchar"]="{0}含有非法字符(例如不能输入空格)!";
ValidationErrorString["zh-cn"]["englishchar"]="{0}只接受英文字符!";
...
ValidationErrorString["en-us"]["phoneNumber"]="{0} is not an valid phone number!";
...
ValidationErrorString["en-us"]["legalchar"]="{0}have some invalid chars(example:some blank)!";
ValidationErrorString["en-us"]["englishchar"]="{0}only accept english chars!";
