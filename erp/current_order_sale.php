...
$dsql->close();
header("Location:current_order_sale.php?action=normal&did=".$rid);
}
?>
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
     ...
     <tr>
      <td bgcolor="#FFFFFF">
	   ...
	   $wsql->Execute();
	   $wrs=$wsql->GetOne();
	   $wsql->ExecuteNoneQuery("insert into #@__sale(productid,number,rdh,dtime) values('".$pid."','".$num."','".$did."','".GetDateTimeMk(time())."')");
	   ...
	   echo "<tr class='row_color_head'><td>货号</td><td>名称</td><td>规格</td><td>分类</td><td>单位</td><td>售价</td><td>供应商<td>入库数量</td><td>删除</tr>";
	   ...
	   $anum+=$row['number'];
	   echo "<tr onMouseMove=\"javascript:this.bgColor='#EBF1F6';\" onMouseOut=\"javascript:this.bgColor='#FFFFFF';\"><td>".$row['productid']."</td><td>&nbsp;".$row1['cp_name']."</td><td>".$row1['cp_gg']."</td><td>".get_name($row1['cp_categories'],'categories').">".get_name($row1['cp_categories_down'],'categories')."</td><td>".get_name($row1['cp_dwname'],'dw')."</td><td>￥".$row1['cp_sale']."</td><td>".$row1['cp_gys']."</td><td>".$row['number']."</td><td><a href='current_order_sale.php?action=del&id=".$row['id']."&rid=".$row['rdh']."'>删除</a></td></tr>";
	   $nsql->close();
	   }
	   }
