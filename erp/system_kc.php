...
$dlist->SetSource($query);
	   ...
	   echo "<tr onMouseMove=\"javascript:this.bgColor='#EBF1F6';\" onMouseOut=\"javascript:this.bgColor='#FFFFFF';\">\r\n<td>".$row['p_id']."</td><td>&nbsp;".$row['cp_name']."</td><td>".$row['cp_gg']."</td><td>".get_name($row['cp_categories'],'categories').">".get_name($row['cp_categories_down'],'categories')."</td><td>".get_name($row['cp_dwname'],'dw')."</td><td>￥".$row['cp_jj']."</td><td>".get_name($row['p_id'],'gys')."</td><td>".get_name($row['l_id'],'lab')."</td><td><font color=red>".$row['number']."</font></td><td><a href='system_kc_edit.php?pid=".$row['cp_number']."&lid=".$row['l_id']."&n=".$row['number']."'>修改</a>|<a href='system_kc_del.php?action=del&id=".$row['cp_number']."'>删除</a><input type='checkbox' name='sel_pro".$row['id']."' value='".$row['id']."'></td>\r\n</tr>";
	   }
