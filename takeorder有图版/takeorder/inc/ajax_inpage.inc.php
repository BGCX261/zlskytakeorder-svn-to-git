...
class show_page {
    ...
    function set($pagesize=20,$total,$current=false,$typeid) {
		...
		$tDiv = $typeid;
         	...
         	$output_tmp="<button style=\"width:auto; height:auto;\" type=\"button\" onclick='lastvalue()'><img src=\"./ico/icon2_089.png\" /></button>&nbsp;&nbsp;&nbsp;";
			 $this->output.=$output_tmp;
		
		if ($this->tpage > 1) {
            if ($current>1) {
				$output_tmp=$this->file.'?'.$this->pvar.'=1'.($this->varstr);
				$output_tmp="<button style=\"width:auto; height:auto;\" onclick='ajax_get(\"$output_tmp\",\"$tDiv\")' type=\"button\"><img src=\"./ico/34aL_002.png\" /></button>&nbsp;&nbsp;&nbsp";
				$this->output.=$output_tmp;
			}
			/*
			if ($current>10) {
      ...
      */
	   ...
	   for ($i=$start; $i<=$end; $i++) {
           ...
           $output_tmp="<button style=\"width:auto; height:auto;\" type=\"button\" onclick='closevalue()'><img src=\"./ico/rest.png\" /></button>";
		   $this->output.=$output_tmp;
	}
