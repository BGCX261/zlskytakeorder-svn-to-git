...
require('config.inc.php');
 define("UserName", $db_username);	//数据库连接用户名	
define("PassWord", $db_password);									//数据库连接密码
define("ServerName", $db_hostname);					//数据库服务器的名称
...
class DBSQL{
	...
	{
		$sql = "SELECT * FROM " . $name . " WHERE k_id = $id";
		$r = $this->select($sql);
		return $r[0];
	}
	/**
	...
	*/
	public function updateData($name,$id,$data){
		...
		$sql = "UPDATE " . $name . " SET " . implode(',',$col) . " WHERE k_id = $id";
	...
	{
		$sql = "DELETE FROM " . $name . " WHERE k_id = $id";
