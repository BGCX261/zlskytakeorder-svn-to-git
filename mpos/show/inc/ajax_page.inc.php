...
class show_page {
    ...
    function set($pagesize=20,$total,$current=false,$typeid) {
		...
		if (!$this->file) {$this->file = "search_list.php";}
		...
		if ($this->tpage > 1) {
            if ($current>1) {
				...
				$output_tmp="<a href=javascript:ajax_get('".$output_tmp."','".$tDiv."')>第一页</a> ";
            ...
            if ($current>1) {
				...
				$output_tmp="<a href=javascript:ajax_get('".$output_tmp."','".$tDiv."')>前一页</a> ";
				$this->output.=$output_tmp;
			}
            ...
            if ($current<$this->tpage) {
				$output_tmp=$this->file.'?'.$this->pvar.'='.($current+1).($this->varstr);
				$output_tmp="<a href=javascript:ajax_get('".$output_tmp."','".$tDiv."')>下一页</a> ";
				$this->output.=$output_tmp;
			}
            ...
            if ($current<$this->tpage) {
				...
				$output_tmp="<a href=javascript:ajax_get('".$output_tmp."','".$tDiv."')>最后页</a> ";
				$this->output.=$output_tmp;
			}
		}
